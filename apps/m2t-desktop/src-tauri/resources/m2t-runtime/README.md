...
Desktop 与 CLI **同一时刻仅一个 monitor owner**（锁文件互斥）。`desktop.auto_start_monitor=true`（默认）时：启动**先检测**已有 owner 则跳过；无监控时 Desktop 自动启内嵌。终端 `monitor watch --daemon` 同样先检测，已有 owner 则退出。切换见 DaemonCard「改用 Desktop / 改用终端」或 `POST /api/runtime/takeover|handoff`。
