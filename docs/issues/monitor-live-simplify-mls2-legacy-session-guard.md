...
github: TBD
branch: issue-TBD-mls2-legacy-session-guard
depends_on: [MLS-1]
---
...
- [ ] `LiveRecordingCore._start_recording`（或 `create_session`）在配置 `effective_pipeline_mode()==legacy` 时拒绝**新** session（清晰错误 / 日志）
- [ ] `snapshot_pipeline_mode()` 在 legacy 配置下仍可为存量场次服务
- [ ] `doctor --json` 对 legacy 配置继续警告
- [ ] 新增 `test_legacy_new_session_rejected`
- [ ] 收窄 `test_live_legacy_pipeline.py`：仅测存量 finalize，不测新 session 创建
