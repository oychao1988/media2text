...
- [x] 新建 helper（`core/live/live_lane.py` 或 `task_scheduler` 模块内）：返回 True 当任一 monitored creator 满足：
...
- [x] 单元测试覆盖「在播无 session」「有 pending prepare」两场景

### Task 2 — scheduler 跳过 post_process drain

- [x] `TaskSchedulerLoop.tick_once`：若 `live_lane_needs_priority` → **跳过** `_post_pool.drain_pending`（segment_pool 仍可 drain）
- [x] log info `post_process_deferred_for_live_lane`（structlog，含计数）
- [x] `test_task_scheduler_defers_post_process_when_live_pending` 通过
