...
- [x] `LiveSessionRepo`、`MonitorTaskRepo`、`StateWriter` 全部 mutating 方法经 `gateway.write` 或 `WriteAwareRepo._mutate`
- [x] `LiveSnapshotRepo.upsert` / `touch_probe` 改经 gateway；**删除** `snapshot.py` 的 `_snapshot_write_lock`
- [x] `BEGIN IMMEDIATE` 仅出现在 gateway writer 线程内（`set_offline_since` / `clear_offline_since`）

### Task 2 — TaskSchedulerLoop

- [x] `TaskSchedulerLoop._run`：`write_batch(tick_once)` 单事务 reconcile + claim + drain 声明（I/O 仍在 worker，claim 在 batch 内）
- [x] 日志 `task_scheduler_db_locked` 在单元压测 30s 内 **0 次**（mock 11 creators + scheduler）

### Task 3 — 测试

- [x] 扩展 `tests/unit/test_task_scheduler.py`：gateway mock 验证 batch 单 commit
- [x] `tests/unit/test_live_db_lock_probe_snapshot.py` 仍 PASS（snapshot 经 gateway）
- [x] 新增 `tests/unit/test_state_writer_gateway.py`：`write_obs` / `set_offline_since` 经 gateway
