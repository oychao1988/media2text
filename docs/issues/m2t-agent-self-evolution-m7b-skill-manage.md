...
- [x] `skill_usage.py`：读写 `{profile}/skills/.usage.json`（`use_count`、`view_count`、`patch_count`、`pinned`、`agent_created` 等，spec §7.4）
- [x] `skills_index.handle_skill_view` 递增 `view_count`
- [ ] skill 加载进 prompt（default_skills / slash）递增 `use_count`（若已有 hook 则接入）
- [x] `skill_manage` 写操作递增 `patch_count`
- [x] bundled `packages/agent-skills/` **不写** telemetry
- [x] `tests/unit/test_skill_usage.py` 通过

### Task 2 — skill_provenance

- [x] `skill_provenance.py`：`ContextVar` `write_origin`（`foreground` | `background_review`）
- [x] background review 路径 `write_origin=background_review`；`create` 时 `mark_agent_created`
- [x] foreground `skill_manage create` **不**标 `agent_created`（S8）
- [x] `tests/unit/test_skill_provenance.py` 通过

### Task 3 — skill_manage 工具

- [x] `skill_manage.py`：`create` / `patch` / `edit` / `delete` / `write_file` / `remove_file`
- [x] 写入根 `{profile_dir}/skills/{name}/SKILL.md`；kebab-case 校验；拒绝 `..` 路径遍历
- [x] bundled skills **只读** → `PROTECTED_SKILL`
- [x] distill `{slug}-perspective`：可 patch/edit/write_file（**禁止** `references/research/*`）；**不可** `delete`；默认 **pinned**（S9）
- [x] `model_tools.py` + `tools/registry.py` 注册 OpenAI schema
- [x] `toolsets.py`：`_HERMES_NAMES` 与 `m2t-core` 默认含 `skill_manage`；review toolset 含 `skill_manage`
- [x] S7：`skill_manage patch` 落盘；`skills_list` 可见
- [x] `tests/unit/test_skill_manage.py` 通过
...
- [x] S11：`write`/`append` memory 仍可用且返回 `deprecated: true`（M7a 若已交付则回归即可）
- [x] `CLAUDE.md` Desktop Agent 小节补充 `skill_manage` 一句（可选，与 M7c 文档一并亦可）
...
- PR: [#219](https://github.com/oychao1988/media2text/pull/219) merged 2026-06-07
