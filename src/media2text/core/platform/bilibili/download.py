...
from media2text.core.platform.bilibili.ytdlp_fallback import (
    download_via_ytdlp_bilibili,
    ytdlp_available,
)
...
def _download_one(
    *,
    ...
    if session_file and session_file.is_file() and ytdlp_available():
        try:
            download_via_ytdlp_bilibili(
                bvid=aweme_id, dest=dest, session_file=session_file
            )
            return aweme_id, True, str(dest)
        except Exception as exc:  # noqa: BLE001
...
def download_pending(
    ...
    *,
    ...
    if creator_id:
        ...
        if creator and creator.platform != "bilibili":
            pending = []
    else:
        pending = [
            r
            for r in pending
            if (c := creators.get(r.creator_id)) and c.platform == "bilibili"
        ]
    ...
    failed = 0
    errors: list[dict] = []
    concurrency = cfg.platforms.bilibili.download_concurrency
