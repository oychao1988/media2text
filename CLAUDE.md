...
**Desktop vs 终端守护**：开机可 `nohup media2text monitor watch --daemon`（或 `bin/monitor-watch-daemon.sh`）独立跑监控，无需开 Desktop。`desktop.auto_start_monitor=true` 时，Desktop/sidecar 启动**不会**抢终端 daemon，UI 显示 `managed_by=external`；无后台监控时才自动启 embedded。切换：`DaemonCard`「改用 Desktop」→ `POST /api/runtime/takeover`；「改用终端」→ `POST /api/runtime/handoff`。
