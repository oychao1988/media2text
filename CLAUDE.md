...
**Desktop vs 终端守护**：同一时刻**仅一个** monitor owner（锁文件互斥）。`desktop.auto_start_monitor=true`（默认）时，Desktop/sidecar 启动**先检测**：已有终端 `monitor watch --daemon` 则**跳过**并显示 `managed_by=external`；无后台监控才自动启内嵌。终端 `monitor watch --daemon` 启动前同样检测，已有 owner 则退出。切换 owner：`DaemonCard`「改用 Desktop」→ `POST /api/runtime/takeover`；「改用终端」→ `POST /api/runtime/handoff`。假锁排错：若 `daemon_lock_pid` 存在但 `daemon_lock_valid=false` / `lock_reason=lock_pid_mismatch`，删除 `data/.monitor-watch.lock` 或开启 `desktop.monitor_self_heal` 后重启 serve；亦可 `POST /api/runtime/takeover`。`bin/monitor-watch-daemon.sh` 启动前会清假锁。

**Sidecar 互斥**：Desktop 启动时会清理其它 `media2text serve` 实例（含未占端口的孤儿进程），并清 stale 的 `data/.serve.lock` / `data/.monitor-watch.lock`；内嵌 monitor 假死（心跳 stale）时启动或 self-heal 会自动 takeover 重启。手动 `media2text serve` 若端口或锁已被占用则退出。
